...
enum commandeType
{
    ERR, // erreur
    WARN, // avertissement
    VAR_DEC, // déclaration de variable
    VAR_DEF, // définition de variable
    OPER, // opération arithmétique
    RET, // return
    AFF, // affectation d'une valeur à une variable
    IF,
    CONDITION
};
...
class Symbole
{
  ...
  private:
    //------------------------------------------------------- Attributs privés
    matrice resp; // liste des commandes du block
    matrice temporalExpression; // liste des commandes temporaire
    ListC *flowControl; // liste des blocks
    Commande temporalStackCommande; // code + liste d'instructions
    map<string, int> tablesDesSymboles; // liste des variables
    int tmpCounter; //
    int index;
